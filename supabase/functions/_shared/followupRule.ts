...
// ── Periodic sweep (the Outlook hole) ──────────────────────────────
//
// `deriveAction` only ever runs when something triggers it: a mail sync, or a
// send. Gmail has a bulk-sync cron that walks EVERY lead of a connection, so a
// quiet Gmail lead is re-derived on a schedule and `followup_due` eventually
// fires. Outlook has no such job — its sync only runs from the UI, and only
// touches leads with new messages. So the post-send recompute this unit added
// is the ONLY recompute an Outlook lead ever gets, and it happens seconds after
// the send, when the wait has obviously not expired and the correct answer is
// "nothing to do". Nothing ever revisits the lead, and the six-week hole stays
// open for exactly the reps this unit exists to help.
//
// `outlook-followup-sweep` closes it by re-deriving the leads that could have
// become due since their last write. This predicate is that selection rule,
// kept pure so it is testable and so the SQL and the in-memory filter can't
// drift.

/**
 * Coarse cut-off for the sweep: the FLOOR of `followupWaitDays`, not its
 * default. The sweep must not second-guess the per-workspace wait — it hands
 * every plausible lead to `deriveAction`, which applies the real number.
 */
export const SWEEP_MIN_QUIET_DAYS = 1;

/** Oldest `last_outbound_at` still too recent to be worth re-deriving. */
export function followupSweepCutoffIso(now: number, quietDays: number = SWEEP_MIN_QUIET_DAYS): string {
  return new Date(now - quietDays * DAY_MS).toISOString();
}

/**
 * Is this lead worth handing to `deriveAction` again?
 *
 * "My message, unanswered, quiet for a while, and currently NOT in the Queue."
 * Leads already surfaced (`needs_action`) need nothing; unsubscribed, closed,
 * permanently-dismissed and snoozed leads must stay out. Mirrors the SQL filter
 * in the sweep so a change to one is caught against the other.
 */
export function isFollowupSweepCandidate(
  lead: {
    ...
    status?: string | null;
    action_permanently_dismissed?: boolean | null;
    action_dismissed_at?: string | null;
  },
  now: number = Date.now(),
  quietDays: number = SWEEP_MIN_QUIET_DAYS,
): boolean {
  if (lead.needs_action === true) return false;            // already in the Queue
  if (lead.unsubscribed === true) return false;
  if (lead.action_permanently_dismissed === true) return false;
  if (lead.action_dismissed_at != null) return false;      // snoozed / handled
  if (lead.status != null && lead.status !== "active" && lead.status !== "new") return false;

  const outbound = lead.last_outbound_at ? new Date(lead.last_outbound_at).getTime() : 0;
  if (!Number.isFinite(outbound) || outbound <= 0) return false;
  if (outbound > now - quietDays * DAY_MS) return false;   // too recent to be due

  const inbound = lead.last_inbound_at ? new Date(lead.last_inbound_at).getTime() : 0;
  return inbound < outbound;                                // mine was the last word
}
