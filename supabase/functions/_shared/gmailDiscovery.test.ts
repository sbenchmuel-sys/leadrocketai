// Run: deno test supabase/functions/_shared/gmailDiscovery.test.ts
//
// BEHAVIOURAL. Deno mirror of src/test/gmailThreadExpansion.test.ts.
//
...
const expect = (actual: any) => ({
  ...
  toContain: (e: unknown) => assertEquals((actual as unknown[]).includes(e), true),
  not: { toContain: (e: unknown) => assertEquals((actual as unknown[]).includes(e), false) },
});
import {
  gmailDirectDiscoveryQuery,
  selectThreadsToExpand,
} from "./gmailDiscovery.ts";
