// Run: deno test supabase/functions/_shared/bounceDisposition.test.ts
//
// BEHAVIOURAL (Unit G-B, findings 3 + QA D1/D2). Deno mirror of
// src/test/bounceDisposition.test.ts — keep the two in step; the vitest one is
// the copy CI actually executes.
//
...
import { bounceDisposition, type BounceDisposition } from "./bounceDisposition.ts";

/** Minimal describe/it shim over Deno.test so this file mirrors the vitest one. */
...
function it(name: string, fn: () => void) {
  Deno.test(`${currentSuite} > ${name}`, fn);
}
