-- 20260511120000_enforce_meeting_workspace_consistency.sql
--
-- Defense-in-depth multi-tenancy guards for the Phase 2 meeting-recap
-- tables. RLS already restricts who can write to which workspace, but
-- nothing currently prevents a buggy writer from inserting a child row
-- whose workspace_id disagrees with the referenced parent's workspace_id.
--
-- Mirrors the existing public.enforce_contact_lead_workspace() pattern
-- from migrations/20260324132908_*.sql, one function per child↔parent
-- FK relationship. Each trigger fires BEFORE INSERT OR UPDATE OF
-- workspace_id + the relevant FK column so that updating workspace_id
-- alone cannot silently move a row cross-tenant.
...
-- meeting_transcript_id is nullable (ON DELETE SET NULL preserves summaries
-- through the 90-day raw-transcript purge); skip the check when it's NULL.
