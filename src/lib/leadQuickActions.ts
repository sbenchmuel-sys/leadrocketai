 ...
 *  - SMS: a phone number exists AND the lead isn't opted out. This opens the rep's
 *    OWN texting app (a manual text), so it's gated on phone + opt-out — NOT on
 *    sms_opted_in (that rule, channels.ts, gates DrivePilot's automated SMS send).
 *  - WhatsApp: a WhatsApp number exists (falls back to the phone number) AND the
 *    lead has opted in to WhatsApp (WhatsApp policy) AND isn't opted out.
