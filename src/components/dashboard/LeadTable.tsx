...
import { BulkMoveToNurtureDialog } from "./BulkMoveToNurtureDialog";
...
export function LeadTable({ leads, isLoading, onLeadUpdated, revenueStateFilter, filters, onFiltersChange, showColumnFilters }: LeadTableProps) {
  ...
  const [bulkNurtureOpen, setBulkNurtureOpen] = useState(false);
  ...
  return (
    <>
          ...
          {selectedLeads.size > 0 && (
              <div className="flex items-center gap-2 animate-fade-in">
                ...
                <Button
                  ...
                  onClick={() => setBulkNurtureOpen(true)}
                >
      ...
      <BulkMoveToNurtureDialog
        selectedLeads={leads.filter((l) => selectedLeads.has(l.id))}
        open={bulkNurtureOpen}
        onOpenChange={setBulkNurtureOpen}
        onSuccess={() => {
          setSelectedLeads(new Set());
          onLeadUpdated?.();
        }}
