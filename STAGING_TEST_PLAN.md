...
### Lead Detail redesign — Unit 4a: "Log a meeting" on the meeting card (PR #117) — Tier 3 (UI relocate)
- What it does: Surfaces the EXISTING manual meeting-log flow on the Lead Detail right rail. The add-meeting form (title + date + notes → AI recap + milestones + follow-up email, then `stage=post_meeting`) was extracted from MeetingsTab into a shared `LogMeetingDialog` used by BOTH the Meetings tab and a new "Log a meeting" button on the Latest Meeting card. Because that card is hidden when there's no meeting yet, the empty state shows a quiet "Log a meeting" rail entry. No new data path; the submit logic is a verbatim move (parity below). The JSON parse helpers moved verbatim to `src/lib/meetingRecap.ts` (shared with the Meetings-tab regenerate path).
- Where: `src/components/lead/{LogMeetingDialog,LeadOverviewPanel,MeetingsTab}.tsx`, `src/lib/meetingRecap.ts`.
- PRs/commits: PR #117.
- Existing tests: `src/lib/__tests__/meetingRecap.test.ts` (recap/milestone JSON parse parity — fenced, truncation-repair, null cases).
- Scenarios:
  - LD4-1 (happy, card present): open a lead that already has a meeting → right rail "Latest Meeting" card. Tap "Log a meeting", enter notes + date, save. Pass when: the dialog opens over the page (rep stays on the lead), the AI recap + milestones generate, the meeting is saved, and the card REFRESHES to the new meeting (and resets "recap pending", not "already sent").
  - LD4-2 (mobile + empty state — the one to eyeball): the right rail is desktop-only (`hidden lg:block`), so "Log a meeting" is exposed TWICE: a quiet entry in the right rail on desktop, AND a `lg:hidden` button in the MAIN column (under the Intelligence card) on phones. Pass when: on a phone (narrow viewport) the main-column "Log a meeting" button is visible + thumb-reachable and opens the dialog — including on a brand-new lead with NO meetings (where the desktop card is hidden); on desktop the rail entry works and the mobile button is hidden. (Without the mobile button, phone reps had no one-tap first-meeting path — Codex P2.)
  - LD4-3 (PARITY — owner's concern): log a meeting from the CARD and, separately, the same notes from More → Meetings ("Log a meeting"). Pass when: both produce the SAME recap bullets, the SAME milestones (the extraction especially), the SAME follow-up email, and both set the lead to post_meeting — they call one shared dialog, so they cannot diverge.
  - LD4-4 (one form): in More → Meetings, the add affordance now opens the dialog (no second inline form); the Meetings-tab "Regenerate recap" on an existing meeting still works (shares the same parse helpers).

### Lead Detail redesign — Unit 4b: "I handled this" + quick WhatsApp/SMS (PR #118) — Tier 3 (UI; reuses existing dismiss + deep-links)
- What it does: Two additions to the full-width lead header (mobile-visible). (1) "I handled this" on the Next-move card dismisses the suggested move WITHOUT sending — reuses the Queue's `markActionHandled(id,{permanent:true})` + 5s Undo toast; sets only `action_permanently_dismissed`, re-armed by `syncEngine` on a fresh inbound; when set (here or from the Queue) the card shows a calm "handled" state instead of nagging. (2) Quick WhatsApp + SMS buttons beside the existing in-app Call (Call unchanged, not duplicated; no Email), opening the rep's OWN apps via `outreachDeepLinks` (wa.me / sms:). Visibility via pure `resolveLeadQuickActions`: hide when number missing; hide both when `unsubscribed`; WhatsApp also needs `wa_opted_in`.
- Where: `src/components/lead/LeadDetailHeader.tsx`, `src/pages/LeadDetail.tsx`, `src/lib/leadQuickActions.ts`; `getLeadDetail` (+whatsapp_number, unsubscribed, action_permanently_dismissed).
- PRs/commits: PR #118.
- Existing tests: `src/lib/__tests__/leadQuickActions.test.ts` (hide-when-missing + opt-out + WhatsApp opt-in).
- Scenarios:
  - LD4B-1 (I handled this — no send): on an action-required lead (Replied Rita), click "I handled this". Pass when: NO email/message is sent and the lead's data is unchanged except the suggestion is dismissed; a 5s Undo toast restores it; the card shows the calm "handled" state; and it re-appears later when a fresh inbound arrives (re-arm). Consistency: a lead marked handled in the Queue shows the same calm state here.
  - LD4B-2 (quick actions hide cleanly): a lead with NO phone and NO whatsapp_number → no Call/WhatsApp/SMS buttons (no dead buttons). A lead with a phone but `wa_opted_in=false` → Text shows, WhatsApp hidden. A lead with phone + `wa_opted_in=true` → both show; tapping opens the texting app / WhatsApp to that number.
  - LD4B-3 (Tier-1-adjacent — OPT-OUT respect): Unsubscribed Uma → NO WhatsApp and NO SMS quick actions appear (opt-out honored). (Call follows its own existing rule.)
  - LD4B-4 (mobile — eyeball): on a phone, both the quick actions and "I handled this" are visible and thumb-reachable in the header (the right rail is desktop-only, so they must be here). Confirm the header action row isn't too crowded with Call + WhatsApp + Text added.
...
### Editable cadence on a saved draft — reconciling step-replace RPC — Tier 1 (step/content reconciliation + ENROLLMENT SAFETY)
- What it does: Lets a rep re-shape a SAVED draft campaign's touches on the detail page (`/app/automations/:id`), reusing the Unit 2 editable `CampaignScript` (add/remove/reorder/retime/change-channel/meeting-link) — not only at first build. The hard part is reconciliation: `campaign_step_content` (generated/edited copy) and `campaign_collateral.attached_step_number` are keyed by `step_number`, so renumbering would mis-point or orphan them. A new SECURITY DEFINER RPC `replace_campaign_steps_reconciled(_campaign_id, _steps)` does it ATOMICALLY in one transaction: replaces the steps (delete-renumber-insert, like `replaceCampaignSteps`) AND remaps the dependents — each surviving touch carries `orig_step_number`, so its copy/links MOVE to the new number (rep edits / `is_edited` / picked options / every industry variant preserved); a REMOVED touch's copy is DELETED and its collateral link falls back to campaign-level (NULL, asset kept); an INSERTED touch starts with NO copy (rep regenerates via "Build the messages"). **Enrollment safety (decided: BLOCK):** structural edits are DRAFT-ONLY. The RPC fail-closes (42501) on any campaign that is not `status='draft'` OR that has ANY `campaign_enrollment`/`campaign_touch` row — so a live/sending campaign (whose per-lead touch schedule is pre-laid-out with `step_number` baked in and advanced by `step_number` lookups) can NEVER be renumbered. Today's read-only behavior for active/paused/completed is preserved both server-side (RPC) and in the UI (`canEditCampaignSteps` gates the "Edit the steps" button on draft + zero cadence rows). Never auto-sends; never calls the AI; reuses the editor (not forked).
- Where: `/app/automations/:id` (`src/pages/CampaignDetail.tsx` — draft-only "Edit the steps" mode). Write path `src/lib/campaignQueries.ts` (`replaceCampaignStepsReconciled` + `campaignHasCadenceRows`). Gate + reconciliation contract `src/lib/campaignStepReconcile.ts`. Editor reused from `src/components/automations/CampaignScript.tsx` + pure helpers in `src/lib/campaignDefaults.ts` (`orig_step_number` rides through the spreads). Migration `20260625000000_replace_campaign_steps_reconciled.sql`.
- PRs/commits: PR editable-cadence-saved-draft (this PR).
- Existing tests: `src/lib/__tests__/campaignStepReconcile.test.ts` (draft-only gate incl. fail-closed on unknown status; `orig_step_number` survives every editor mutation; reconciliation map for reorder / remove / insert / combined). `campaignPlanEditing.test.ts` stays green (editor unchanged).
- Scenarios:
  - EC-1 (Tier 1 — reorder renumbers AND reconciles copy): On a Dealership A DRAFT with generated copy for steps 1–3, reorder so step 3 moves up to slot 2 and save. Pass when: `campaign_steps` are renumbered 1..N with the new order; each surviving step's `campaign_step_content` now sits at its NEW number (the copy followed the touch — old step 3's copy is at step 2), no row points at a wrong step, and `is_edited`/picked-option/variant rows are preserved.
  - EC-2 (Tier 1 — remove drops only that step's copy): remove a middle touch from a draft and save. Pass when: that step's `campaign_step_content` rows are DELETED, the later steps' copy slides up to its new number, surviving copy is intact, and any collateral linked to the removed step now reads campaign-level (`attached_step_number` NULL) with the asset itself kept.
  - EC-3 (Tier 1 — insert starts blank, no auto-AI/auto-send): insert a new touch mid-plan and save. Pass when: the inserted step has NO `campaign_step_content` row (rep regenerates via "Build the messages"); no AI call fired on save; `send_mode` unchanged; nothing sent.
  - EC-4 (Tier 1 — ENROLLMENT SAFETY, the load-bearing block): take a campaign that is `status='active'` (or has any `campaign_enrollment`/`campaign_touch` row) and call `replace_campaign_steps_reconciled` directly (and confirm the UI shows NO "Edit the steps" button + read-only script). Pass when: the RPC raises `42501` and makes ZERO changes to `campaign_steps`/`campaign_step_content`/`campaign_touch`; in-flight per-lead touches are untouched; no send is affected.
  - EC-5 (Tier 1 — ISOLATION): a Dealership B rep calls the RPC for a Dealership A draft. Pass when: `42501 not authorized` (the `is_workspace_member` gate inside the DEFINER function), zero rows changed; the editor never renders for a campaign outside the rep's workspace.
  - EC-6 (atomic — no orphan window): if the steps replace fails (e.g. a CHECK violation), pass when: the whole transaction rolls back — `campaign_step_content` is NOT left renumbered against un-renumbered steps (the reconciliation and the step replace either both land or both don't).
  - EC-7 (cap respected): an edit that would push the plan past 10 steps is refused (editor caps the "Add a step" affordance; the RPC raises "too many steps"). Pass when: a draft can never exceed 10 steps via this path.
  - EC-8 (Tier 1 — channel change invalidates channel-shaped copy; Codex P2): on a draft with generated copy, change a saved EMAIL touch (has subject/body) to a CALL and save, then open "Build the messages". Pass when: the old email copy did NOT carry onto the call (no stale subject/body on the call row — its content identity was dropped), the call starts blank, and "Build the messages" actually generates the call talking-points/voicemail (it is NOT skipped as "already has content"). Reverse (call→email) likewise yields a blank email that regenerates, never a body-less email at send. UNDO (Codex P2): changing the channel and then changing it BACK to the original before saving must PRESERVE the original copy (no avoidable deletion) — identity is decided at save by channel-match, not destroyed on the intermediate edit.
  - EC-9 (Tier 1 — enroll-vs-edit race, both directions; Codex P1): a draft is edited (renumbered) at the same time as people are enrolled into it (two tabs / two reps). Pass when: there is NEVER a `campaign_touch` row whose `step_number` is absent from the renumbered `campaign_steps`. Verify BOTH guards: (a) if enrollment commits first, the edit RPC's post-write re-check sees the new enrollment/touch rows and raises (the renumber rolls back, "people were enrolled while you were editing"); (b) if the edit commits first, enrollment's step-fingerprint re-read before inserting touches detects the change, rolls back its just-created enrollment (clearing only the campaign_ids it stamped), and asks the rep to retry. Also: the edit RPC takes `FOR UPDATE` on the campaign row so two simultaneous edits (or an edit racing activation) serialize.
  - EC-10 (UI gate refresh; Codex P2): on a draft detail page, click "Add people" and enroll someone WITHOUT navigating away. Pass when: the "Edit the steps" button disappears immediately (the cadence-row gate re-reads after enroll) instead of staying visible and only failing at save; removing the last enrolled person re-enables it.
