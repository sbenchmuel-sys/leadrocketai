...
- Scenarios:
  ...
  - LD4-2 (empty state — the one to eyeball): open a brand-new lead with NO meetings. Pass when: a quiet "Log a meeting" entry is visible in the right rail (the card itself stays hidden), it's thumb-reachable on a phone, and tapping it opens the same dialog. (Without this, a new lead has nowhere to log its first meeting except More → Meetings.)
