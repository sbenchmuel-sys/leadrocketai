...
- **Known deliberate deviations (do NOT file as bugs):** email/call auto-deletion is widened to 30 days on purpose (the 72h purge erased timeline history / broke replies — see QA-FOLLOWUPS.md). Staging cron jobs are not fully replicated yet. The `campaign-collateral` storage bucket is intentionally `public=true` (public-by-link read; member-scoped write) — see OD-3; verified 2026-06-17.
