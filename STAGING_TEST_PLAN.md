...
### Outreach Unit 3 — generation reads the per-step meeting flag — Tier 1 (email send)
- What it does: Makes `campaign_steps.include_meeting_cta` the per-step source of truth at GENERATION time (Unit 2 only stored it). The resolver collapses the tri-state via `resolveStepMeetingCta` (`true`→force on, `false`→off, `null`→today's default) and exposes `meeting_cta_enabled` on the resolved instruction. The live sender (`automation-executor`) threads the **lead-owner's own** `rep_profiles.calendar_link` into the draft (`meeting_link` + the rep-context Calendar Link line) **unless** the step is explicitly `false`; a `true` step also gets a structural hard rule so the link survives even the intro template's "no calendar links unless instructed" guard. The authoring **preview** (`resolveCampaignAuthoringInstruction` → `ai_task`) now loads the **requesting rep's own** link and makes the SAME per-step decision via the shared `meetingLinkForDraft` helper, so preview and live send agree. A typed instruction like "add the meeting CTA to every email" is a shortcut (`detectMeetingCtaIntent`/`applyMeetingCtaIntent`) that pre-ticks every email step's box; specific-email or soft asks set nothing (rep uses the checkboxes). Per-rep only, fail-closed; no calendar_link → CTA omitted cleanly (no placeholder). **A `null` flag is byte-identical to today (golden-tested).**
- Where: Background (`automation-executor`, `ai_task` authoring branch). Resolver `supabase/functions/_shared/campaignResolver.ts` + client mirror `src/lib/campaignResolver.ts`; loader `campaignStepLoader.ts`/`campaignStepConfig.ts`; authoring `aiCampaignResolver.ts`; shortcut `src/lib/campaignDefaults.ts`; wired in `NewCampaign.tsx` Step 2.
- PRs/commits: PR per-step-meeting-cta (this PR).
- Existing tests: `src/lib/__tests__/perStepMeetingCta.test.ts` (vitest, 21), `supabase/functions/_shared/campaignMeetingCta.test.ts` (Deno, 6 — incl. cross-rep no-leak), golden tests stay green (null path byte-identical).
- Scenarios:
  - PSM-1 (Tier 1 — happy): On Eligible Ed's active campaign, tick "Include a meeting link" on emails 2 and 3 only; leave 1 and 4 unticked-as-OFF. Trigger generation (manual invoke / review mode). Pass when: drafts for emails 2 and 3 contain Ed-rep's `calendar_link`; emails 1 and 4 contain NO booking link.
  - PSM-2 (Tier 1 — instruction shortcut): New outreach, instructions include "add the meeting link to every email". Build → Step 2. Pass when: every EMAIL step's "Include a meeting link" box is pre-ticked (calls/texts untouched); after save+generate, every email draft carries the rep's link.
  - PSM-3 (Tier 1 — byte-unchanged): A campaign whose email steps are all left as default (null). Pass when: generated drafts are unchanged from before this PR — the rep's link is still offered where the template already offered it (follow-ups), intro still holds back. No new links appear or disappear on null steps.
  - PSM-4 (Tier 1 — no calendar link): A rep with an empty `rep_profiles.calendar_link`, email step flagged ON. Pass when: the draft has NO booking link and NO placeholder/broken link text — the CTA is simply omitted.
  - PSM-5 (Tier 1 — ISOLATION, cross-rep no-leak): Dealership A rep and Dealership B rep each have their own `calendar_link`. Generate for an A-owned lead and a B-owned lead on the same campaign. Pass when: each draft contains ONLY its own lead-owner's link — never the other rep's. In the authoring preview, each rep sees only their OWN link.
  - PSM-6 (Tier 1 — preview == live): Author a campaign as Rep A with email step 2 flagged ON; view the cadence preview, then send live. Pass when: the preview draft and the live-sent draft make the SAME meeting-CTA decision (both include the link on step 2; both omit on a step flagged OFF).
  - PSM-7 (meeting already booked): A lead with `has_future_meeting=true` on a flagged step. Pass when: the draft does NOT ask for a new meeting (existing "meeting already booked" suppression still fires) even though the link may be present.
  - PSM-8 (auto-send gating unchanged — REGRESSION): cold-send consent gate, suppression/unsubscribe floor, OOO/dedup are all unchanged. Pass when: OC-2/OC-3 still hold; this PR touches only which link (if any) is threaded, never WHETHER a send is allowed.
  - PSM-9 (manual Queue draft — KNOWN LIMITATION, not a defect): generating a one-off draft from the Queue (not the campaign auto-send) does NOT yet consult the per-step flag — it keeps today's behavior (the rep's own link is offered as before). Don't file this as "false flag ignored". The per-step flag governs campaign auto-send + the authoring preview.
