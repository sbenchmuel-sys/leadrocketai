...
- **Call audio + transcripts: 90-day purge exists but is SHIPPED OFF.** Be precise about this — it is a public commitment that is not yet being kept. `purge_call_media()` (migration `20260908150000_purge_call_media.sql`) nulls recording URLs/paths, all transcript text forms and the analysis `signals_json` once a session is older than its workspace's `call_settings.audio_retention_days` (default 90). Its pg_cron job `call-media-purge` is created with **`active = false`**; until the founder runs `UPDATE cron.job SET active = true WHERE jobname = 'call-media-purge';` **nothing is purged and call media is retained indefinitely.** Two further caveats even once enabled: (a) it clears the DB pointers, it does NOT yet delete the objects in the `call-recordings` Storage bucket — that sweeper is still a follow-up; (b) `call_analyses.summary_short` / `summary_long` are durable AI paraphrases and are kept indefinitely by design, matching the email rule. Verify retention logic before changing the call pipeline.
...
- **`vite.config.ts` hardcodes PRODUCTION Supabase as a build fallback** — `cloudUrl`/`cloudPublishableKey` default to the prod project (`ntzeiflqqluwgdfmatjh`) and a `define` injects them, which **overrides `.env*` files**. A build without `VITE_SUPABASE_URL` in `process.env` (e.g. `vite build --mode staging` on its own) **silently ships production**. For a staging build, export the staging vars first: `set -a; . ./.env.staging; set +a` — always grep the bundle for the intended ref before deploying. See `STAGING_TEST_PLAN.md` → "Staging deployment".
...
- **Type-check:** `npx tsc -b --noEmit` (the `-b` is load-bearing — the root tsconfig is a solution file, plain `tsc --noEmit` checks nothing). **Staging preflight:** `npm run preflight:staging` (read-only secret probes; `--print` dry run). Exact commands, when each suite is mandatory, and the deploy commands → `STAGING_TEST_PLAN.md` → "Master-upgrade gate: commands".

**Four rules every PR follows (the PR template checks them):**
1. `src/integrations/supabase/types.ts` is generated by Lovable — never hand-edit it (use a narrow local type or a cast with a `ponytail:` comment until types regenerate).
2. One migration per PR, named `supabase/migrations/<YYYYMMDDHHMMSS>_<slug>.sql`; the PR body says "Apply migration <file>" or "no migration".
3. Every `supabase functions deploy` / `supabase db push` carries an explicit `--project-ref` (`supabase/config.toml` points at production; a bare command is a review blocker).
4. The staging cron file (`*_codify_cron_jobs_staging.sql`) is applied on staging only; production keeps the plain `*_codify_cron_jobs.sql` lineage.

**Cross-runtime shared code:** pure modules in `supabase/functions/_shared/` (no `Deno.*`, no `esm.sh`, no `import.meta.env`) can be imported from `src/` via the `@shared/*` alias; `src/test/sharedPurity.test.ts` and `src/test/noDuplicateBasenames.test.ts` enforce the boundary.
